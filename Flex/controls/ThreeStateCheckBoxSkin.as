package com.db.rds.content.ExceptionManagement.view.controls 
{
	...
	/**
	 *  The skin for all the states of the icon in a ExtendedCheckBox
	 */
	public class ThreeStateCheckBoxSkin extends Border 
	{
		 ...
		 */
		private static function calcDerivedStyles(themeColor: uint, borderColor: uint, fillColor0: uint,
		...
		public function ThreeStateCheckBoxSkin() {
			super();
		}
		...
		protected override function updateDisplayList(w: Number, h: Number): void {
			...
			var checkColor: uint = getStyle("iconColor"); // added style prop
			var middleCheckColor: uint = getStyle("middleIconColor"); // added style prop
			
			if (isNaN(getStyle("middleIconColor"))) {
				middleCheckColor = 0x666666;
			}
			...
			switch (name) {
				...
				case "middleUpIcon":  {
					...
					// border
					drawRoundRect(0, 0, w, h, 0, [borderColor, borderColorDrk1], 1, verticalGradientMatrix(0, 0, w, h), GradientType.LINEAR, null, {x: 1, y: 1, w: w - 2, h: h - 2, r: 0});
					...
					// top highlight
					drawRoundRect(1, 1, w - 2, (h - 2) / 2, 0, [0xFFFFFF, 0xFFFFFF], highlightAlphas, verticalGradientMatrix(1, 1, w - 2, (h - 2) / 2));
				...
				case "middleOverIcon":  {
					...
					// border
					drawRoundRect(0, 0, w, h, 0, [themeColor, themeColorDrk1], 1, verticalGradientMatrix(0, 0, w, h), GradientType.LINEAR, null, {x: 1, y: 1, w: w - 2, h: h - 2, r: 0});
				...
				case "disabledIcon":  {
					...
					disFillAlphas = [Math.max(0, fillAlphas[0] - 15), Math.max(0, fillAlphas[1] - 15)];
					
					// border
					drawRoundRect(0, 0, w, h, 0, [borderColor, borderColorDrk1], 0.5, verticalGradientMatrix(0, 0, w, h), GradientType.LINEAR, null, {x: 1, y: 1, w: w - 2, h: h - 2, r: 0});
