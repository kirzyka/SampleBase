...
var currentDate = null;// = null;//DATES[0].date / 1000;
var currentTime = null;// = null;
...
var _Supplier = {
	...
	getItem: function(list) {
        ...
        if(getUrlVars()[PATH_SUPPLIER] === undefined) {
            ...
            result = getItemById(list, $.cookie(CKS_SUPPLIER));
        } else {
            // set supplier from path param
            result = getItemById(list, getUrlVars()[PATH_SUPPLIER]);
        }
...
var _Location = {
	...
	getItem: function(list) {
        ...
        if(getUrlVars()[PATH_LOCATION] === undefined) {
            ...
            result = getItemById(list, $.cookie(CKS_LOCATION));
        } else {
            // get location from path param
            result = getItemById(list, getUrlVars([PATH_LOCATION]);
        }
        if(result == null && list != null && list.length > 0) { 
        	result = list[0]; 
        }
...
var _Date = {
	...
	getItem: function(list) {
        ...
        if(getUrlVars()[PATH_DATE] === undefined) {
            ...
            result = getItemByValue(list, $.cookie(CKS_DATE));
        } else {
            // get date from path param
            result = getItemByValue(list, getUrlVars()[PATH_DATE]);
        }
        if(result == null && list != null && list.length > 0) { 
        	result = list[0]; 
        }
...
var _Time = {
	...
	getItem: function(list) {
        ...
        if(getUrlVars()[PATH_TIME] === undefined) {
            ...
            result = getItemById(list, $.cookie(CKS_TIME));
        } else {
            // get time from path param
            result = getItemById(list, getUrlVars()[PATH_TIME]);
        }
        if(result == null && list != null && list.length > 0) { 
        	result = list[0]; 
        }
...
var _Basket = {
	...
	saveBaskets: function(baskets) {
	    if (typeof localStorage !== "undefined") {
	        localStorage.setItem(CKS_BASKET, baskets);
	    } else {
	        $.cookie(CKS_BASKET, baskets);
	    }
	},
...
$(document).ready(function () {
    ...
    /*// Listners
    $(__CurrentSupplier__).change(getLocations());
    $(__CurrentLocation__).change(getDates());
    $(__CurrentDate__).change(getTimes()); */
