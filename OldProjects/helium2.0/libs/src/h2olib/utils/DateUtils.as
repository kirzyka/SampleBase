package h2olib.utils
{
	...
	public class DateUtils
	{
		...
		/**
		 *  Constants contain all days of the week.
		 */
		...
		/**
		 *  Constants contain all months of the year.
		 */
		...
		/**
		 *  Constants contain time in milliseconds by day intervals
		 *  such as minute, hour and day.
		 *  These constants are internal values for using in date/time
		 *  calculations.
		 */
		...
		/**
		 *  This property is a generic object for holding day of
		 *  the week values.
		 *
		 *  @default null
		 */
		private static var _daysOfWeek:Object = null;
		/**
		 *  This property is internal variable used in date/time
		 *  formatting.
		 */
		private static var _dateFormatter:DateFormatter;
		/**
		 *  This property is a generic object for holding month
		 *  values.
		 *
		 *  @default null
		 */
		...
		/**
		 *  The internal variable used in date/time
		 *  formatting.
		 */
		...
		public static function convert(inputDate:String):String
	  	{
	    if (!inputDate) return "";
	    var date:Date = DateField.stringToDate(inputDate, DATE_SERVER_FORMAT);
	    if (!date) return "";
	    return DateField.dateToString(date, DATE_CLIENT_FORMAT);
	    }
		
		/**
		 *  A generic object for holding day of the week values.
		 */
		public static function get daysOfWeek():Object
		{
			if (!_daysOfWeek)
			{
				_daysOfWeek = {};
				_daysOfWeek[DateUtils.SUNDAY]    = 0;
				_daysOfWeek[DateUtils.MONDAY]    = 1;
				_daysOfWeek[DateUtils.TUESDAY]   = 2;
				_daysOfWeek[DateUtils.WEDNESDAY] = 3;
				_daysOfWeek[DateUtils.THURSDAY]  = 4;
				_daysOfWeek[DateUtils.FRIDAY]    = 5;
				_daysOfWeek[DateUtils.SATURDAY]  = 6;
			}
			return _daysOfWeek;
		}
		/**
		 *  A generic object for holding month in the year values.
		 */
		 ...
		 *  @private
		 *
		 *  This function will remove any invalid characters
		 *  from the date/time mask based upon a pattern.
		 *
		 *  @param mask - the string for matching;
		 *  @param pattern - the valid characters for this
		 *         mask;
		 *  @param defaultValue - the default value to return
		 *         to the calling page should the mask not match
		 *         the pattern.
		 *
		 *  @return a validated <code>mask</code> based upon the
		 *          original pattern.
		 */
		...
		/**
		 * Subs the specified number of "date parts" to a date, e.g. 6 days
		 *
		 * @param datePart	The part of the date that will be added
		 * @param number	The total number of "dateParts" to add to the date
		 * @param date		The date on which to add
		 ...
		 * @return			The new date
		 */
		public static function dateSub( datePart:String, number:Number, date:Date ):Date {
			var _returnDate:Date = new Date( date );
		...
		public static function dayOfWeekAsNumber( strDayOfWeek:String ):Number {
			return (daysOfWeek[ strDayOfWeek ] >= 0 ) ? daysOfWeek[ strDayOfWeek ] : -1;
		}
		...
		public static function getMaxDate(date1:Date, date2:Date):Date
		{
		...
		public static function getMinDate(date1:Date, date2:Date):Date
		{
			if(date1.time - date2.time > 0)
			{
				return date2;
		...
		public static function getDateString_ServerFormat(value:String):String
		{
			return DateField.dateToString(DateField.stringToDate(value, GlobalSettings.DATE_SERVER_FORMAT),GlobalSettings.DATE_FORMAT);
		}
	}
}